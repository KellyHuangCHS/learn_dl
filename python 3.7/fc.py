...
class Network(object):
    ...
    def one_train_sample(self, label, sample, rate):
        self.predict(sample)
        ...
        for fc in self.layers:
            for i in range(fc.W.shape[1]):
                for j in range(fc.W.shape[1]):
                    ...
                    fc.W[i,j] += epsilon
                    print('weights:(%d,%d): expected - actual %.4e - %.4e' % (i, j, expect_grad, fc.W.grad[i,j]))
...
def correct_radio(network):
    normalizer = Normalizer()
    correct = 0.0
    for i in range(256):
        if normalizer.denorm(network.predict(normalizer.norm(i))) == i:
            correct += 1.0
    print('correct_radio: %.2f%%'%(correct / 256 * 100))
